...
def validate_cwd(workspace: Path, cwd: str) -> Path:
    candidate = (workspace / (cwd.strip() or ".")).resolve()
    try:
...
def docker_command(argv: list[str], workspace: Path, cwd: Path) -> list[str]:
...
def create_workspace() -> WorkspaceResponse:
    ...
    (SANDBOX_ROOT / workspace_id).mkdir(mode=0o700)
...
def execute(workspace_id: str, request: ExecuteRequest) -> ExecuteResponse:
    ...
    try:
        completed = subprocess.run(
            docker_command(argv, workspace, cwd),
