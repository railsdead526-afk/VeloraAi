...
def register_platform_tools(registry) -> None:
    definitions = [
        ...
        ToolDefinition(name="github_read_file", description="Read a file from a repository. Read-only operation.", handler=github_read_file, allowed_plans=READ_PLANS, parameters={"type":"object","properties":{"repository":{"type":"string"},"path":{"type":"string"}},"required":["repository","path"],"additionalProperties":False}, timeout_seconds=15, max_calls_per_request=5),
        ...
        ToolDefinition(name="terminal_run_build", description="Run a build command in the isolated sandbox.", handler=terminal_run_build, allowed_plans=TERMINAL_PLANS, parameters={"type":"object","properties":{"command":{"type":"string"},"cwd":{"type":"string"},"timeout":{"type":"integer","minimum":1,"maximum":60}},"required":["command"],"additionalProperties":False}, requires_confirmation=True, timeout_seconds=65, max_calls_per_request=2),
        ToolDefinition(name="terminal_install_package", description="Install a package inside the isolated sandbox.", handler=terminal_install_package, allowed_plans=TERMINAL_PLANS, parameters={"type":"object","properties":{"manager":{"type":"string","enum":["npm","pnpm","yarn","pip"]},"package":{"type":"string"},"cwd":{"type":"string"},"timeout":{"type":"integer","minimum":1,"maximum":120}},"required":["manager","package"],"additionalProperties":False}, requires_confirmation=True, timeout_seconds=125, max_calls_per_request=2),
    ]
